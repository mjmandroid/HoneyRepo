package com.beautystudiocn.allsale.vendor.ptr;
...
import com.beautystudiocn.allsale.vendor.ptr.indicator.PtrIndicator;
import com.beautystudiocn.allsale.vendor.ptr.util.PtrCLog;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    /*** 是否处理滑动事件 ***/
    private boolean mIsConsumeMotionEvent = true;
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.lib_PtrFrameLayout, 0, 0);
        if (arr != null) {
            mHeaderId = arr.getResourceId(R.styleable.lib_PtrFrameLayout_lib_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(R.styleable.lib_PtrFrameLayout_lib_ptr_content, mContainerId);
            mFooterId = arr.getResourceId(R.styleable.lib_PtrFrameLayout_lib_ptr_footer, mFooterId);

            mPtrIndicator.setResistanceHeader(arr.getFloat(R.styleable.lib_PtrFrameLayout_lib_ptr_resistance, mPtrIndicator.getResistanceHeader()));
            mPtrIndicator.setResistanceFooter(arr.getFloat(R.styleable.lib_PtrFrameLayout_lib_ptr_resistance, mPtrIndicator.getResistanceFooter()));
            mPtrIndicator.setResistanceHeader(arr.getFloat(R.styleable.lib_PtrFrameLayout_lib_ptr_resistance_header, mPtrIndicator.getResistanceHeader()));
            mPtrIndicator.setResistanceFooter(arr.getFloat(R.styleable.lib_PtrFrameLayout_lib_ptr_resistance_footer, mPtrIndicator.getResistanceFooter()));

            mDurationToBackHeader = arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_duration_to_back_refresh, mDurationToCloseHeader);
            mDurationToBackFooter = arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_duration_to_back_refresh, mDurationToCloseHeader);
            mDurationToBackHeader = arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_duration_to_back_header, mDurationToCloseHeader);
            mDurationToBackFooter = arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_duration_to_back_footer, mDurationToCloseHeader);

            mDurationToCloseHeader = arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_duration_to_close_either, mDurationToCloseHeader);
            mDurationToCloseFooter = arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_duration_to_close_either, mDurationToCloseFooter);
            mDurationToCloseHeader = arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_duration_to_close_header, mDurationToCloseHeader);
            mDurationToCloseFooter = arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_duration_to_close_footer, mDurationToCloseFooter);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(R.styleable.lib_PtrFrameLayout_lib_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable.lib_PtrFrameLayout_lib_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(R.styleable.lib_PtrFrameLayout_lib_ptr_pull_to_fresh, mPullToRefresh);

            mMode = getModeFromIndex(arr.getInt(R.styleable.lib_PtrFrameLayout_lib_ptr_mode, 4));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeaderView != null && mPtrIndicator != null) {
        ...
        if (mFooterView != null && mPtrIndicator != null) {
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT && mPtrIndicator != null) {
    ...
    private void layoutChildren() {
        ...
        if (mContent != null) {
            ...
            int right;
            int bottom;
            if (mPtrIndicator != null && mPtrIndicator.isHeader()) {
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        if (!isEnabled() || !mIsConsumeMotionEvent || mContent == null || mHeaderView == null) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (!mForceBackWhenComplete) {
                    ...
                    boolean isFooter = mPtrIndicator != null && !mPtrIndicator.isHeader() && mPtrIndicator.hasLeftStartPosition(); // if the footer is showing
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                float offsetX = mPtrIndicator != null ? mPtrIndicator.getOffsetX() : 0;
                float offsetY = mPtrIndicator != null ? mPtrIndicator.getOffsetY() : 0;
                ...
                boolean canMoveUp = mPtrIndicator != null && mPtrIndicator.isHeader() && mPtrIndicator.hasLeftStartPosition(); // if the header is showing

                boolean canMoveDown = mFooterView != null && mPtrIndicator != null && !mPtrIndicator.isHeader() && mPtrIndicator.hasLeftStartPosition(); // if the footer is showing
    ...
    private void onRelease(boolean stayForLoading) {
        ...
        if (mStatus == PTR_STATUS_LOADING) {
            ...
            if (mKeepHeaderWhenRefresh) {
                // scroll header back
                if (mPtrIndicator != null && mPtrIndicator.isOverOffsetToKeepHeaderWhileLoading() && !stayForLoading) {
                    if (mScrollChecker != null) {
                        if (mPtrIndicator.isHeader()) {
    ...
    private void tryScrollBackToTop() {
        if (mScrollChecker != null && !mPtrIndicator.isUnderTouch() && mPtrIndicator.hasLeftStartPosition()) {
            mScrollChecker.tryToScrollTo(PtrIndicator.POS_START, mPtrIndicator.isHeader() ? mDurationToCloseHeader : mDurationToCloseFooter);
            return;
        }

        if (mScrollChecker != null && mForceBackWhenComplete && !mPtrIndicator.isHeader() && mStatus == PTR_STATUS_COMPLETE) {
    ...
    private void performRefresh() {
        ...
        if (mPtrUIHandlerHolder != null && mPtrUIHandlerHolder.hasHandler()) {
    ...
    protected void onPtrScrollAbort() {
        if (mPtrIndicator != null && mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker != null && mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
    ...
    public float getResistanceHeader() {
        return mPtrIndicator != null ? mPtrIndicator.getResistanceHeader() : 0;
    }

    @SuppressWarnings({"unused"})
    public float getResistanceFooter() {
        return mPtrIndicator != null ? mPtrIndicator.getResistanceFooter() : 0;
    }
    ...
    public int getOffsetToRefresh() {
        return mPtrIndicator != null ? mPtrIndicator.getOffsetToRefresh() : 0;
    }
    ...
    public float getRatioOfHeaderToHeightRefresh() {
        return mPtrIndicator != null ? mPtrIndicator.getRatioOfHeaderToHeightRefresh() : 0;
    }

    @SuppressWarnings({"unused"})
    public int getOffsetToKeepHeaderWhileLoading() {
        return mPtrIndicator != null ? mPtrIndicator.getOffsetToKeepHeaderWhileLoading() : 0;
    }
    ...
    class ScrollChecker implements Runnable {
        ...
        public void run() {
            boolean finish = mScroller != null && (!mScroller.computeScrollOffset() || mScroller.isFinished());
            int curY = mScroller != null ? mScroller.getCurrY() : 0;
            int deltaY = curY - mLastFlingY;
            if (DEBUG && mPtrIndicator != null) {
                if (deltaY != 0) {
                    PtrCLog.v(LOG_TAG,
        ...
        public void tryToScrollTo(int to, int duration) {
            if (mPtrIndicator != null && mPtrIndicator.isAlreadyHere(to)) {
                return;
            }
            mStart = mPtrIndicator != null ? mPtrIndicator.getCurrentPosY() : 0;
            mTo = to;
    ...
    /**
     * @return 内部可滑动子控件是否滑动到顶部
     */
    public boolean checkContentViewScrollTop() {
        return mPtrHandler != null && mContent != null && mHeaderView != null && mPtrHandler.checkCanDoRefresh(this, mContent, mHeaderView);
    }

    /**
     * @return 内部可滑动子控件是否滑动到底部
     */
    public boolean checkContentViewScrollBottom() {
        return mPtrHandler != null && mContent != null && mFooterView != null && mPtrHandler instanceof PtrHandler2 && ((PtrHandler2) mPtrHandler).checkCanDoLoadMore(this, mContent, mFooterView);
    }

    public void setConsumeMotionEvent(boolean consumeMotionEvent) {
        mIsConsumeMotionEvent = consumeMotionEvent;
    }
}
