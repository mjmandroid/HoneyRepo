...
public class ValidateUtil {
     ...
     * @return
     */
    public static boolean isTextValidate(String text, String textName, int minLen, int maxLen) {
        if (TextUtils.isEmpty(text)) {
            ToastUtil.showToast(textName + "不能为空");
            return false;
        }
        if (text.length() > maxLen) {
            ToastUtil.showToast(textName + "不能超过" + maxLen + "字符");
            return false;
        }
        if (text.length() < minLen) {
            ToastUtil.showToast(textName + "不能少于" + minLen + "字符");
            return false;
        }
     ...
     * @return boolean
     * @author longluliu
     * @date 2014-9-3 下午9:01:01
     */
    public static boolean isValidateEmpty(String text, String textName) {
        if (TextUtils.isEmpty(text)) {
            ToastUtil.showToast(textName + "不能为空");
            return false;
        }
